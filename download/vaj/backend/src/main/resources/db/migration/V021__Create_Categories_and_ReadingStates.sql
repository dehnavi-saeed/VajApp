...
CREATE INDEX IX_Categories_DisplayOrder
...
CREATE INDEX IX_BookCategories_CategoryId
    ON BookCategories(CategoryId);


-- V023__Create_Reading_States.sql
...
CREATE TABLE ReadingStates (
                               ...
                               CONSTRAINT UQ_ReadingStates_BookId
                                   UNIQUE(BookId),

                               CONSTRAINT FK_ReadingStates_Books
                                   FOREIGN KEY (BookId)
                                       REFERENCES Books(Id)
);

CREATE INDEX IX_ReadingStates_UserId
    ON ReadingStates(UserId);

CREATE INDEX IX_ReadingStates_Status
    ON ReadingStates(Status);